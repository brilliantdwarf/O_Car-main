...
static float err_anAcc_y = 0;
static float err_acc_y = 0;
static float err_gyro_y = 0;
static float err_anAcc_z = 0;
...
static float aSintegError_z = 0;
static float aSintegError_y = 0;
int8 dir = 1;
...
#if     aS2out
    // pitch 轴 —— Y 轴 角速度环 -------------------------------------------------------------------------------- //
    if(func_abs((int)(100 * (angleSpeedY_set - gyroYArray_avge[10]))) > 1)
    {
...
//           duty_out_gyro *= 0.5;
    ...
    duty_out_gyro = err_gyro_y * 14;//28;
...
//        angle_z = 0;
    if(func_abs(100 * (angleZ_set - 6 * angle_z)) > 1)
    {
...
//           duty_out_acc *= 0.8;
    ...
    aSpeed_out_BALANCE_acc = err_acc_z * 3.8;// 3.9 ~ 4.0 也可以貌似 //2.8;//2.5;// 3
//    aSpeed_out_BALANCE_acc += (err_acc_z - lastErr_acc) * 2.3;

//    if(func_abs(10 * err_acc_z) < 55)
//    {
       integError_z += err_acc_z * 0.2;
//       aSpeed_out_BALANCE_acc += integError_z;
//    }
//    else
//       integError_z *= 0.9;

    integError_z = func_limit_ab(integError_z, -4, 4);
...
//       integError_y *= 0.9;
    ...
    // speed to angle ---------------------------------------外外环------------------------------------------------- //
#if     Speed2angle
//    sum_Speed_Lef = (Speed_Lef_set - SpeedLef_avge[5]) * func_abs(angle_z) * 0.005;//0.08;//0.062;//0.15;
//    sum_Speed_Rig = (Speed_Rig_set + SpeedRig_avge[5]) * func_abs(angle_z) * 0.005;//0.08;//0.062;//0.15;
    sum_Speed_Lef = (Speed_Lef_set - SpeedLef_avge[5]) * 0.8;//0.08;//0.062;//0.15;
    sum_Speed_Rig = (Speed_Rig_set + SpeedRig_avge[5]) * 0.8;//0.08;//0.062;//0.15;
    sum_Speed_Turn = (sum_Speed_Lef + sum_Speed_Rig) / 2;
//    sum_Speed_Turn = func_limit_ab(sum_Speed_Turn, -5, 5);
//    sum_Speed_Turn = func_limit_ab(sum_Speed_Turn, -5, 5);
...
void balance4( void )
{
    ...
    // angular acceleration -------------------------------内内环------------------------------------------------ //
#if     acc2out
    // pitch 轴 —— Y 轴 角加速度环 ------------------------------------------------------------------------------ //

    if(10 * func_abs(anaccY_set - (gyroYArray_avge[10] - last_gyro)) > 5)
    {
        err_anAcc_y = anaccY_set - (gyroYArray_avge[10] - last_gyro);
    }
    else
    {
        duty_out_anAcc *= 0.8;
    ...
    duty_out_anAcc += err_anAcc_y * 150;
    last_gyro = gyroYArray_avge[10];
#else
#endif
...
//           duty_out_gyro *= 0.5;
    ...
    duty_out_gyro = err_gyro_y * 0.028;//0.01;

    aSintegError_y += err_gyro_y * 0.008;
    aSintegError_y = func_limit_ab(aSintegError_y, -0.15, 0.15);
    duty_out_gyro += aSintegError_y;

    anaccY_set = func_limit_ab(duty_out_gyro, -30, 30);
...
//    duty_out_turn = func_limit_ab(duty_out_turn, -1000, 1000);
    ...
    // Z 方向偏航角 用 Y 方向角速度弥补 角度环 --------------------------------------------------------------------- //
//        angle_z = 0;
    if(func_abs(10 * (angleZ_set - angle_z)) > 5)
    {
       err_acc_z = angleZ_set - angle_z;
    }
    else
...
//        duty_out_gyro -= sum_Speed_Turn;
    ...
    duty_out = func_limit_ab(duty_out_anAcc, -8500, 8500);
